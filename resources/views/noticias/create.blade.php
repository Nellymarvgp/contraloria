...
@section('header', 'Crear Nueva Noticia')
        ...
        <form action="{{ route('noticias.store') }}" method="POST" enctype="multipart/form-data" id="createNoticiaForm">
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('titulo') border-red-500 @enderror"
                    id="titulo" type="text" name="titulo" value="{{ old('titulo') }}" required>
            </div>
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" name="publicado" value="1" class="form-checkbox h-5 w-5 text-blue-600" {{ old('publicado') ? 'checked' : '' }}>
...
<script>
document.getElementById('createNoticiaForm').addEventListener('submit', function(e) {
