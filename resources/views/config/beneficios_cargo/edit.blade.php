...
@section('title', 'Editar Beneficio por Cargo')
@section('header', 'Editar Beneficio por Cargo')
    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Editar Beneficio por Cargo</h2>

                <form method="POST" action="{{ route('beneficios-cargo.update', $beneficioCargo) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="beneficio_id" id="beneficio_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            @foreach($beneficios as $beneficio)
                                <option value="{{ $beneficio->id }}" {{ old('beneficio_id', $beneficioCargo->beneficio_id) == $beneficio->id ? 'selected' : '' }}>
                                    {{ $beneficio->beneficio }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="cargo" id="cargo" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <option value="Todos" {{ old('cargo', $beneficioCargo->cargo) == 'Todos' ? 'selected' : '' }}>Todos</option>
                            <option value="Alto funcionario" {{ old('cargo', $beneficioCargo->cargo) == 'Alto funcionario' ? 'selected' : '' }}>Alto funcionario</option>
                            <option value="Alto Nivel" {{ old('cargo', $beneficioCargo->cargo) == 'Alto Nivel' ? 'selected' : '' }}>Alto Nivel</option>
                            <option value="Empleado" {{ old('cargo', $beneficioCargo->cargo) == 'Empleado' ? 'selected' : '' }}>Empleado</option>
                            <option value="Obrero" {{ old('cargo', $beneficioCargo->cargo) == 'Obrero' ? 'selected' : '' }}>Obrero</option>
                        </select>
                        @error('cargo')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div x-data="{ valorTipo: '{{ old('valor', $beneficioCargo->valor) !== null && old('valor', $beneficioCargo->valor) !== '' ? 'valor' : 'porcentaje' }}' }" class="mb-4">
                        <div class="mb-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div x-show="valorTipo === 'porcentaje'" x-cloak>
                                ...
                                <input
                                    ...
                                    value="{{ old('porcentaje', $beneficioCargo->porcentaje) }}"
                            ...
                            <div x-show="valorTipo === 'valor'" x-cloak>
                                ...
                                <input
                                    ...
                                    value="{{ old('valor', $beneficioCargo->valor) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                >
                                @error('valor')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
