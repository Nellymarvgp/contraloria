        ...
        <form action="{{ route('remuneraciones.update', $remuneracion->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <select name="tipo_personal" id="tipo_personal" 
                    ...
                    <option value="administracion_publica" {{ old('tipo_personal', $remuneracion->tipo_personal) == 'administracion_publica' ? 'selected' : '' }}>Funcionarios y funcionarias</option>
                    <option value="obreros" {{ old('tipo_personal', $remuneracion->tipo_personal) == 'obreros' ? 'selected' : '' }}>Personal obrero</option>
                </select>
                @error('tipo_personal')
            ...
            <div id="funcionarios_fields" style="display: none;">
                ...
                <div class="mb-4">
                    ...
                    <select name="tipo_cargo" id="tipo_cargo"
                        ...
                        <option value="">Seleccione un tipo</option>
                        <option value="bachiller" {{ old('tipo_cargo', $remuneracion->tipo_cargo) == 'bachiller' ? 'selected' : '' }}>Bachiller</option>
                        <option value="tecnico_superior" {{ old('tipo_cargo', $remuneracion->tipo_cargo) == 'tecnico_superior' ? 'selected' : '' }}>Técnico Superior Universitario</option>
                        <option value="profesional_universitario" {{ old('tipo_cargo', $remuneracion->tipo_cargo) == 'profesional_universitario' ? 'selected' : '' }}>Profesional Universitario</option>
                    </select>
                    @error('tipo_cargo')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select name="grupo_cargo_id" id="grupo_cargo_id" 
                        ...
                        @foreach($gruposCargos as $grupo)
                            <option value="{{ $grupo->id }}" data-categoria="{{ $grupo->categoria }}" {{ (old('grupo_cargo_id', $remuneracion->grupo_cargo_id) == $grupo->id) ? 'selected' : '' }}>
                                {{ $grupo->descripcion }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div id="obrero_fields" style="display: none;">
                <div class="mb-4">
                    ...
                    <select name="clasificacion" id="clasificacion" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50 @error('clasificacion') border-red-500 @enderror">
                        ...
                        <option value="no_calificados" {{ old('clasificacion', $remuneracion->clasificacion) == 'no_calificados' ? 'selected' : '' }}>No calificados</option>
                        <option value="calificados" {{ old('clasificacion', $remuneracion->clasificacion) == 'calificados' ? 'selected' : '' }}>Calificados</option>
                        <option value="supervisor" {{ old('clasificacion', $remuneracion->clasificacion) == 'supervisor' ? 'selected' : '' }}>Supervisor</option>
                    </select>
                    @error('clasificacion')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    ...
                    <select name="grado" id="grado" class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50 @error('grado') border-red-500 @enderror">
                        ...
                        @for($i = 1; $i <= 10; $i++)
                            <option value="{{ $i }}" {{ old('grado', $remuneracion->grado) == $i ? 'selected' : '' }}>{{ $i }}</option>
                        @endfor
                    </select>
                    @error('grado')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div class="mb-4">
                    <label for="valor" class="block text-gray-700 font-medium mb-2">Valor</label>
                    <input type="number" step="0.01" name="valor" id="valor_obrero" value="{{ old('valor', $remuneracion->valor) }}" 
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 focus:ring-opacity-50 @error('valor') border-red-500 @enderror">
                    @error('valor')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
