...
@section('title', 'Editar Grupo o Clase de Cargo')

@section('header', 'Editar Grupo o Clase de Cargo')

@section('content')
<div class="bg-white shadow-md rounded-lg overflow-hidden">
    <div class="py-4 px-6 bg-gray-100 border-b">
        <h2 class="text-xl font-semibold text-gray-800">Editar Grupo o Clase de Cargo</h2>
    </div>

    <div class="py-4 px-6">
        <form action="{{ route('grupos-cargos.update', $grupo->id) }}" method="POST">
            @csrf
            @method('PUT')
            
            <div class="mb-4">
                <label for="descripcion" class="block text-gray-700 font-medium mb-2">Descripción</label>
                <input type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $grupo->descripcion) }}" 
            ...
            <div class="mb-4">
                <label class="flex items-center">
                    <input type="checkbox" name="estado" id="estado" {{ $grupo->estado ? 'checked' : '' }}
