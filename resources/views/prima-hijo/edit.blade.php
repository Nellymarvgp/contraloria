...
@section('title', 'Editar Prima por Hijo')

@section('header', 'Editar Prima por Hijo')

@section('content')
<div class="bg-white shadow-md rounded-lg overflow-hidden">
    <div class="py-4 px-6 bg-gray-100 border-b">
        <h2 class="text-xl font-semibold text-gray-800">Editar Prima por Hijo</h2>
    </div>

    <div class="py-4 px-6">
        <form action="{{ route('prima-hijo.update', $prima->id) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="hijos" class="block text-gray-700 font-medium mb-2">Número de hijos</label>
                <input type="number" name="hijos" id="hijos" min="1" value="{{ old('hijos', $prima->hijos) }}"
