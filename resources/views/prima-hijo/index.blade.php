...
@section('title', 'Prima por Hijo')

@section('header', 'Prima por Hijo')

@section('content')
<div class="bg-white shadow-md rounded-lg overflow-hidden">
    <div class="py-4 px-6 bg-gray-100 border-b flex justify-between items-center">
        <h2 class="text-xl font-semibold text-gray-800">Listado de Primas por Hijo</h2>
        <a href="{{ route('prima-hijo.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded">
        ...
        <table id="datatable" class="min-w-full display nowrap" style="width:100%">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <th class="py-3 px-6 text-left text-xs font-medium text-gray-600 uppercase">Número de hijos</th>
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="py-2 px-6 text-sm">{{ $prima->hijos }}</td>
                    ...
                    <td class="py-2 px-6 text-sm flex">
                        <a href="{{ route('prima-hijo.edit', $prima->id) }}" class="text-blue-500 hover:text-blue-700 mr-3">
                            <i class="fas fa-edit"></i>
                        </a>
                        <form action="{{ route('prima-hijo.destroy', $prima->id) }}" method="POST" class="inline" onsubmit="return confirm('¿Estás seguro de que deseas eliminar esta prima por hijo?');">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="5" class="py-6 px-6 text-center text-gray-500">No hay primas por hijo registradas</td>
                </tr>
                @endforelse
            </tbody>
        </table>
